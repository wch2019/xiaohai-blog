...
public class UserController {
    ...
    @Operation(summary = "新增用户表",security = {@SecurityRequirement(name = "authentication")})
    @PostMapping()
    public Response<Integer> add(@RequestBody UserVo vo){
        return  Response.success("新增用户表成功！", serService.add(vo));
    }

    @Operation(summary = "删除用户表",security = {@SecurityRequirement(name = "authentication")})
    @DeleteMapping("{id}")
    public Response<Integer> delete(@PathVariable("id") Long id){
        return  Response.success("删除用户表成功！",serService.delete(id));
    }

    @Operation(summary = "更新用户表",security = {@SecurityRequirement(name = "authentication")})
    @PutMapping()
    ...
    @Operation(summary = "id查询用户表",security = {@SecurityRequirement(name = "authentication")})
    @GetMapping("{id}")
    public Response<User> findById(@PathVariable Long id){
        return  Response.success("id查询用户表成功！",serService.findById(id));
    }

    @Operation(summary = "查询用户表列表数据",security = {@SecurityRequirement(name = "authentication")})
