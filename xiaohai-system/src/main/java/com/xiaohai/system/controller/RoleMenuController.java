...
public class RoleMenuController {
    ...
    @Operation(summary = "新增角色权限关联表",security = {@SecurityRequirement(name = "authentication")})
    @PostMapping()
    public Response<Integer> add(@RequestBody RoleMenuVo vo){
        return  Response.success("新增角色权限关联表成功！", oleMenuService.add(vo));
    }

    @Operation(summary = "删除角色权限关联表",security = {@SecurityRequirement(name = "authentication")})
    @DeleteMapping("{id}")
    public Response<Integer> delete(@PathVariable("id") Long id){
        return  Response.success("删除角色权限关联表成功！",oleMenuService.delete(id));
    }

    @Operation(summary = "更新角色权限关联表",security = {@SecurityRequirement(name = "authentication")})
    @PutMapping()
    ...
    @Operation(summary = "id查询角色权限关联表",security = {@SecurityRequirement(name = "authentication")})
    @GetMapping("{id}")
    public Response<RoleMenu> findById(@PathVariable Long id){
        return  Response.success("id查询角色权限关联表成功！",oleMenuService.findById(id));
    }

    @Operation(summary = "查询角色权限关联表列表数据",security = {@SecurityRequirement(name = "authentication")})
